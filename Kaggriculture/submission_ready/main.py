"""Candidate: opponents/submission_30 + retry-on-shortfall for
HIRE/BUY_LAND/BUY_ANIMAL/BUY_SEED (`_purchase_retry`, below `agent`).

Round 13 (see opponents/README.md and reports/latest_analysis.md):
rebased off submission_30 instead of continuing on submission_29
(round 10's base). Both are frozen scripts with the same weed-repair
catch-up window, but submission_30 has no mirror-defense machinery at
all -- just `_impact_slots`, a price-impact reorder of existing SELL
orders (never resizing the order list) so a turn that exceeds the
10-order cap truncates the least valuable sells first, not whoever
happens to be listed last. Its resource allocation also matches the
real #1 leaderboard player's logs (round 12) almost exactly: 14 hands,
8 cow + 6 sheep, 2 land purchases, WHEAT=92/MELON=24/STRAWBERRY=42 seed
totals. Measured head-to-head, submission_30 beats submission_29 by
~13.6% cleanly (5 seeds: $136,872 vs. $120,524 mean) -- a real, sizeable
gap from resource allocation alone, not a market-timing trick.

The one addition here: `_purchase_retry`, ported over from the
submission_29-based predecessor (opponents/README.md's "Round 11") --
submission_30 has the identical fixed-cost-purchase-silently-fails
vulnerability, so the same fix applies unchanged in spirit (adapted to
this file's `_seat`/`_farm` helpers).
...
def _current_price(obs, item):
    market = _get(obs, "market", {}) or {}
    prices = _get(market, "prices", {}) or {}
    price = _get(prices, item, None)
    ...
    inventory = _get(market, "inventory", {}) or {}
    try:
        return float(_market_price(item, int(_get(inventory, item, 10000) or 0)))
    except Exception:
        return 0.0


def _purchase_retry(obs, action, step):
    """Adapted from submissions/candidate/main.py's submission_29-based
    predecessor (opponents/README.md's "Round 11") to this file's
    _seat/_farm helpers. HIRE/BUY_LAND/BUY_ANIMAL/BUY_SEED have fixed
    per-unit costs and, in the real env, simply no-op if money is short
    that exact turn; a frozen script has no way to notice or recover, so
    one bad cash-flow moment permanently loses that purchase for the rest
    of the game (BUY_LAND is the worst case: it strands an entire
    quadrant's later scripted actions on LOCKED tiles). This checks live
    affordability before submitting them and queues any shortfall to
    retry on later turns instead of letting the real env silently drop it.

    Two fixes over the submission_29-based version, both needed because
    submission_30 interleaves SELL/BUY_PRODUCT with purchase-type orders
    within the same turn far more than submission_29 did:

    1. This turn's own orders are processed *in place* -- never pulled
       out and re-appended after whatever's left (only backlogged
       shortfalls from *previous* turns are appended at the end, since
       those have no position in this turn's list to begin with). The
       submission_29-based version's partition-and-reconcatenate approach
       silently reordered orders relative to interleaved SELL/BUY_PRODUCT
       entries, changing which one got first claim on money that turn.
    2. The running money estimate now updates live as a SELL (adds
       estimated revenue) or BUY_PRODUCT (subtracts estimated cost) order
       is walked past, using `_current_price`, matching the real engine's
       sequential per-slot-index processing -- a purchase-type order
       listed *after* a SELL in the same turn correctly sees that SELL's
       cash. The submission_29-based version only ever read the turn's
       starting money, so any purchase after a same-turn SELL looked less
       affordable than it really was.

    Fixing only (1) left the regression basically unchanged (mean
    $104,461 vs. baseline's $143,506, still ~27%); tracing a specific
    diverging turn found a `BUY_ANIMAL` silently dropped even though the
    turn had a `SELL` earlier in the same list that would have covered
    it -- (2) fixes that directly. Both together: see the validation
    numbers in opponents/README.md's "Round 13".
    """
    seat = _seat(obs)
    ...
    # Backlog first (oldest shortfalls get first claim on money), using
    # whatever room this turn's own (order-preserving) entries don't need.
    backlog_out = []
    backlog_room = max(0, 10 - len(original))
    for op, item, qty in queue:
        if backlog_room <= 0:
            new_queue.append((op, item, qty))
            continue
        qty_capped = min(qty, backlog_room) if op in _MULTI_UNIT_OPS else qty
        n = spend(op, item, qty_capped)
        if n > 0:
            backlog_out.extend(_emit_purchase(op, item, n))
            backlog_room -= n if op in _MULTI_UNIT_OPS else 1
        leftover = qty - n
        if leftover > 0:
            new_queue.append((op, item, leftover))
    ...
    for order in original:
        ...
        if op in ("HIRE", "BUY_LAND"):
            ...
            if n > 0:
                ...
                new_queue.append((op, None, 1))
            continue
        if op in ("BUY_ANIMAL", "BUY_SEED") and len(order) >= 3:
            item = order[1]
            try:
                qty = int(order[2])
            except (TypeError, ValueError):
        ...
        if op == "SELL" and len(order) >= 3:
            try:
                ...
                qty = 0
            money += _current_price(obs, order[1]) * qty
            out.append(order)
            continue
    ...
    action["market"] = (out + backlog_out)[:10]
    return action
